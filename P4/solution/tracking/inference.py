...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particles_list = [ p for p in self.legalPositions] * (self.numParticles/len(self.legalPositions))
        
        for p in self.legalPositions:
            if len(self.particles_list) == self.numParticles:
                 break
            else:
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            allPossible[self.getJailPosition()] = 1
            self.particles_list = [self.getJailPosition()] * self.numParticles
            self.beliefs = allPossible
            return
        ...
        for p in self.particles_list:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            if emissionModel[trueDistance] > 0:
                allPossible[p] += emissionModel[trueDistance] #OR : emissionModel[trueDistance] * self.beliefDist[p]
        ...
        if allPossible.totalCount() == 0:
            ...
            for i in range(self.numParticles):
                self.particles_list.append(util.sample(allPossible, ))
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles_list:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            new_particles_list.append(util.sample(newPosDist, ))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        Cartesian = list(itertools.product(self.legalPositions, repeat= self.numGhosts))
        random.shuffle(Cartesian)
        self.particles_list = [p for p in Cartesian] * (self.numParticles/ len(Cartesian))
    ...
    def elapseTime(self, gameState):
        ...
        """
        newParticles = []
        for oldParticle in self.particles_list:
        ...
        self.particles_list = newParticles
